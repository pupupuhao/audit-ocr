...
from run_vl_eval import process_pdf_vl
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode",
        choices=["direct-vl", "auto-vl"],
        default="direct-vl",
        help="direct-vl runs VL on every selected page; auto-vl screens pages first. Default: direct-vl",
    )
...
def run_one_pdf(pdf_path: Path, output_root: Path, args: argparse.Namespace) -> dict[str, Any]:
    if args.mode == "direct-vl":
        return process_pdf_vl(
            pdf_path,
            output_root,
            args.dpi,
            args.max_pages,
...
def main() -> None:
    ...
    try:
        ...
        print(f"ERROR: {exc}")
        write_json(reports_dir / "batch_ocr_summary.json", {"mode": args.mode, "file_count": 0, "errors": [str(exc)]})
        return
    ...
    if not pdf_files:
        ...
        write_json(
            reports_dir / "batch_ocr_summary.json",
            {"mode": args.mode, "file_count": 0, "missing": missing, "files": []},
        )
        return

    print(f"Batch OCR mode={args.mode} dpi={args.dpi} files={len(pdf_files)}")
    ...
    write_json(reports_dir / "batch_ocr_summary.json", batch_summary)
    print(f"\nDone. Batch OCR summary written to {reports_dir / 'batch_ocr_summary.json'}")
