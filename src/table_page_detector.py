...
def _target_structure_hits(compact_text: str) -> list[str]:
    checks = [
        ...
        ("quantity_confirm_columns", ["名称", "维修内容", "工程量"]),
        ("quantity_confirm_columns", ["序号", "维修内容", "单位"]),
        ("unit_project_fee_columns", ["费用名称", "计算公式", "金额"]),
        ("specialty_fee_columns", ["工程名称", "金额", "暂估价"]),
        ("resource_labor_columns", ["工日", "名称", "单价"]),
        ("resource_material_columns", ["主要材料", "一览表"]),
        ("resource_material_columns", ["工程设备", "一览表"]),
        ("resource_machine_columns", ["机械台班", "一览表"]),
        ("resource_machine_columns", ["主要机械", "台班", "一览表"]),
    ]
    hits = []
...
def detect_target_table(ocr_result: dict[str, Any] | None) -> dict[str, Any]:
    ...
    blacklist_hits = sorted(set(_keyword_hits(compact, TARGET_TABLE_BLACKLIST)))
